...
struct RuntimeData
{
	enum Index
	{
		...
		Caller,
		Origin,
		TransferredCallValue,	// actual vapor amount transferred
...
struct JITSchedule
{
	...
	VVMJIT_API int64_t id() const;

	/// @returns an identifier for the code that is built from the code and the schedule data.
	VVMJIT_API std::string codeIdentifier(h256 const& _codeHash) const;
};
...
public:
	...
	VVMJIT_API ~ExecutionContext() noexcept;
...
public:
	...
	/// Ask JIT if the VVM code is ready for execution.
	/// Returns `true` if the VVM code has been compiled and loaded into memory.
	...
	VVMJIT_API static bool isCodeReady(std::string const& _codeIdentifier);

	/// Compile the given VVM code to machine code and make available for execution.
	VVMJIT_API static void compile(
		byte const* _code,
		uint64_t _codeSize,
	...
	VVMJIT_API static ReturnCode exec(ExecutionContext& _context, JITSchedule const& _schedule);
};
...
namespace std
{
template<> struct hash<dev::vvmjit::h256>
{
	size_t operator()(dev::vvmjit::h256 const& _h) const
	{
