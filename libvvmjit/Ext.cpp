...
{
	static const auto funcName = "vvm.exists";
...
{
	static const auto funcName = "vvm.sload";
	auto func = _module->getFunction(funcName);
	if (!func)
	{
		// TODO: Mark the function as pure to eliminate multiple calls.
...
{
	static const auto funcName = "vvm.sstore";
	auto func = _module->getFunction(funcName);
	if (!func)
	{
...
{
	static const auto funcName = "vvm.balance";
	auto func = _module->getFunction(funcName);
	if (!func)
	{
		auto addrTy = llvm::IntegerType::get(_module->getContext(), 160);
...
{
	static const auto funcName = "vvm.code";
	auto func = _module->getFunction(funcName);
	if (!func)
	{
		auto addrTy = llvm::IntegerType::get(_module->getContext(), 160);
...
{
	static const auto funcName = "vvm.selfdestruct";
	auto func = _module->getFunction(funcName);
	if (!func)
	{
		auto addrPtrTy = llvm::Type::getIntNPtrTy(_module->getContext(), 160);
...
{
	static const auto funcName = "vvm.log";
	auto func = _module->getFunction(funcName);
	if (!func)
	{
		auto addrPtrTy = llvm::Type::getIntNPtrTy(_module->getContext(), 160);
...
llvm::Function* getCallFunc(llvm::Module* _module)
{
	...
	if (!func)
	{
		...
		auto fty = llvm::FunctionType::get(
			...
			false);
		func = llvm::Function::Create(fty, llvm::Function::ExternalLinkage, "vvm.call", _module);
		func->addAttribute(4, llvm::Attribute::ReadOnly);
		func->addAttribute(4, llvm::Attribute::NoAlias);
		func->addAttribute(4, llvm::Attribute::NoCapture);
		func->addAttribute(5, llvm::Attribute::ReadOnly);
		...
		auto notDelegateCall = builder.CreateICmpNE(&callKind, builder.getInt32(VVM_DELEGATECALL));
		...
		auto failRet = builder.CreateOr(&gas, builder.getInt64(VVM_CALL_FAILURE));
...
{
	static const auto funcName = "vvm.blockhash";
	auto func = _module->getFunction(funcName);
	if (!func)
	{
		// TODO: Mark the function as pure to eliminate multiple calls.
												   ...
												   llvm::Value* _initSize)
{
	...
	auto ret = createCABICall(
		func, {getRuntimeManager().getEnvPtr(), m_builder.getInt32(VVM_CREATE),
