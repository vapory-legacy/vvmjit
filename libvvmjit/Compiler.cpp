...
Compiler::Compiler(Options const& _options, vvm_revision _rev, bool _staticCall, llvm::LLVMContext& _llvmContext):
	m_options(_options),
	m_rev(_rev),
...
void Compiler::resolveJumps()
{
	...
	// Iterate through all VVM instructions blocks (skip first one and last 4 - special blocks).
								 ...
								 Arith256& _arith, Memory& _memory, Ext& _ext, GasMeter& _gasMeter)
{
	...
	for (auto it = _basicBlock.begin(); it != _basicBlock.end(); ++it)
	{
		...
		case Instruction::CREATE:
		{
			...
			llvm::Value* gasKept = (m_rev >= VVM_TANGERINE_WHISTLE) ?
			...
			auto rmagic = m_builder.CreateSelect(
				ret, m_builder.getInt64(0), m_builder.getInt64(VVM_CALL_FAILURE),
				"call.rmagic");
			// TODO: optimize
		...
		case Instruction::STATICCALL:
		{
			...
			if (inst == Instruction::DELEGATECALL && m_rev < VVM_HOMESTEAD)
				goto invalidInstruction;

			if (inst == Instruction::STATICCALL && m_rev < VVM_BYZANTIUM)
				goto invalidInstruction;
			...
			if (m_rev >= VVM_TANGERINE_WHISTLE)
			{
				auto gas = _runtimeManager.getGas();
			...
			int kind = [inst]() -> int
			{
				switch (inst)
				{
				case Instruction::CALL: return VVM_CALL;
				case Instruction::CALLCODE: return VVM_CALLCODE;
				case Instruction::DELEGATECALL: return VVM_DELEGATECALL;
				case Instruction::STATICCALL: return VVM_STATICCALL;
		...
		case Instruction::REVERT:
		{
			...
			if (isRevert && m_rev < VVM_BYZANTIUM)
				goto invalidInstruction;
